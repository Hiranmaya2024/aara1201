document.addEventListener('DOMContentLoaded', async () => {
  ...
  //  welcomeMessage.textContent = `Hello, ${username}!`;
    ...
    if (userLedger.length > 0) {
        ...
        // Calculate the sum of row[4] where row[8] >= 30
        // Calculate the sum of row[4] where row[8] >= 30
    //const sumFilteredSales = filteredSales
   // .filter(row => row[8] >= 30)
   // .reduce((sum, row) => sum + parseFloat(row[4]), 0);
  //  console.log('Sum of filtered sales where row[8] >= 30:', sumFilteredSales);

// Assuming sumFilteredSalesElement is the HTML element where you want to display the result
//const sumFilteredSalesElement = document.getElementById('default-due');
//if (sumFilteredSalesElement) {
 //   sumFilteredSalesElement.textContent = `Sum of filtered sales where row[8] >= 30: ${sumFilteredSales}`;
//} else {
 //   console.error('Element with ID "default-due" not found.');
//}
...
const customername = document.getElementById('customer-nam');
...
if (!customername || !address || !phone || !shopName || !tourday || !nextVisit) {
...
try {
    ...
    if (userLedgr) {
        ...
        customername.textContent = userLedgr[8] || 'No Data found.';
        ...
        customername.textContent = 'No Data found.';
    ...
    customername.textContent = 'Error fetching data.';
